...
# review interval + last_reviewed (creation date)
...
for idx, (maj_q, det_q) in enumerate(STEP_QIDS, start=1):
    ...
    if not maj_raw:
        continue  # skip entirely if no major text for this step
    ...
    # extract bullet items from major column
    bullet_items = [
        line.strip()[2:].strip()
    ...
    # Detail column: leave empty (no "N/A") when blank
